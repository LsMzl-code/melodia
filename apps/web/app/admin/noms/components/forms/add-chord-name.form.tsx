...
const AddChordNameForm = () => {
  ...
  const onSubmit = async (data: z.infer<typeof NameSchema>) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/chord-names/create`, data, {
        method: 'POST', headers: {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="border border-foreground/10 p-5 rounded-xl space-y-5">
        <h2 className="text-2xl font-medium mb-5">Ajouter un nom d'accord</h2>
...
export default AddChordNameForm