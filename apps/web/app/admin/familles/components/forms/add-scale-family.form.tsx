...
const AddScaleFamilyForm = () => {
  ...
  const onSubmit = async (data: z.infer<typeof FamilySchema>) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/scale-families/create`, data, {
        method: 'POST', headers: {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="border border-foreground/10 p-5 rounded-xl space-y-5">
        <h2 className="text-2xl font-medium mb-5">Ajouter une famille de gammes</h2>
...
export default AddScaleFamilyForm