...
import { NotesColumns, TonalitiesColumns } from "./Columns"
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  //*** ACTIONS ***//
  const handleDeleteTonality = async (tonalityId: number) => {
    try {
      ...
      await axios.delete(`http://localhost:8000/tonalities/delete/${tonalityId}`, {
    ...
    <Popover>
      ...
      <PopoverContent className="p-0 w-[150px] bg-background text-gray-50 border-foreground/10">
        ...
        <div className="flex flex-col text-sm p-1">
          ...
          <DeleteDialog
            element="cette tonalité"
            action={() => handleDeleteTonality(data.id)}
          />
